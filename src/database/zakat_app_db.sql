CREATE DATABASE
    IF NOT EXISTS `zakat_app_db` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT ENCRYPTION = 'N';
...
CREATE VIEW `ACCEPTED_DONATIONS` AS 
	SELECT
	    1 AS `id`,
	    1 AS `amount`,
	    1 AS `address`,
...
CREATE TABLE
    `area` (
        ...
        CONSTRAINT `chairpersonId` FOREIGN KEY (`chairpersonId`) REFERENCES `user` (`id`) ON DELETE
        SET
            NULL
...
CREATE TRIGGER `AREA_BEFORE_INSERT` BEFORE INSERT ON 
`AREA` FOR EACH ROW BEGIN 
...
CREATE TRIGGER `AREA_BEFORE_UPDATE` BEFORE UPDATE ON 
`AREA` FOR EACH ROW BEGIN 
...
CREATE VIEW `AREA_WITH_CHAIRPERSON` AS 
	SELECT
	    1 AS `id`,
	    1 AS `createdAt`,
	    1 AS `updatedAt`,
...
CREATE VIEW `CHAIRPERSON_DATA` AS 
	SELECT
	    1 AS `id`,
	    1 AS `firstName`,
	    1 AS `lastName`,
...
CREATE VIEW `COLLECTED_DONATIONS` AS 
	SELECT
	    1 AS `id`,
	    1 AS `amount`,
	    1 AS `address`,
...
VALUES (
        '100392530859786283',
        15000,
        'F-14, Plot BS-9, Block-18, F.B. Area, Karachi',
        'PENDING',
        NULL,
        NULL,
        '100392530859786282',
        ...
        '100402492583444528',
        50000,
        'Flat F-14, Samanabad Mubarace Corner, F.B. Area Karachi',
        'COLLECTED',
        NULL,
        NULL,
        '100392530859786282',
...
CREATE TRIGGER `DONATION_BEFORE_INSERT` BEFORE INSERT 
ON `DONATION` FOR EACH ROW BEGIN 
	SET NEW.id = GENERATE_ID();
	SET NEW.createdAt = CURRENT_TIMESTAMP();
	END 
; 
...
CREATE TRIGGER `DONATION_BEFORE_UPDATE` BEFORE UPDATE 
ON `DONATION` FOR EACH ROW BEGIN 
	SET NEW.updatedAt = CURRENT_TIMESTAMP();
	END 
; 
...
CREATE VIEW `REQUESTED_DONATIONS` AS 
	SELECT
	    1 AS `id`,
	    1 AS `amount`,
	    1 AS `address`,
...
VALUES (
        ...
        'Test',
        'Donor',
        'testdonor123@gmail.com',
        '5366',
        0,
        'DONOR',
        '+923121212121',
        '4210112121212',
        'f4006ded3a51071a9d94a26f0d5bc869c8f92298',
        1,
        NULL,
        NULL
    ), (
        ...
        'Test',
        'Super',
        'testsuper1@gmail.com',
        '3336',
        0,
        'GENERAL_SECRETARY',
        '+923232323232',
        '4210123232323',
        '37483bf780aa68fbb00b20e5f2dbef28f07d48f6',
        1,
        NULL,
        NULL
    ), (
        ...
        'Test',
        'Worker2',
        'testworker2@gmail.com',
        '9859',
        0,
        'WORKER',
        '+923076452378',
        '4210132323232',
        '438267d774d5abd60c7545544d26f191f407da25',
        1,
        '100392530859786250',
        '2023-07-10 10:11:31'
    ), (
        ...
        'Daniyal',
        'Aamir',
        'daniyal.amir110@gmail.com',
        NULL,
        1,
        'DONOR',
        '+923042868395',
        '4210183876527',
        '9da390506134d0bfbf00ca43e12d25342c968624',
        1,
        NULL,
        NULL
    ), (
        ...
        '+923045868286',
        '4210156896325',
        'c4977c27ca021c8c8e6bd4d022a5ca9d9ee66bd0',
        1,
        NULL,
        '2023-07-12 09:07:32'
    ), (
        ...
        '+923181245345',
        '4210165386509',
        'dc9d9f9e46afedb291882d76cfe06c687fc278b3',
        1,
        '100392530859786250',
        '2023-07-16 07:08:11'
    ), (
        ...
        'Hamza',
        'Hassan',
        'hamzahassan@gmail.com',
        '8259',
        0,
        'DONOR',
        '+923458765425',
        '4210157845236',
        'd55bdbcfd025baa67249f976d64dfe31a84d7f89',
        1,
        NULL,
        '2023-07-18 21:03:58'
    ), (
        ...
        '+923435876549',
        '4210185769532',
        '6e91f581d5e6f8780dfe6f7ec98aee2b62507980',
        1,
        '100392530859786250',
        '2023-07-19 06:13:13'
    ), (
        ...
        'Maham',
        'Aamir',
        'mahamamir842@gmail.com',
        '5962',
        0,
        'DONOR',
        '+923458565896',
        '4210156854236',
        'e7c6c5cc4690126532a4d44cfa8660fd5fda0180',
        1,
        NULL,
        '2023-07-19 06:16:15'
    ), (
        ...
        '+923191296169',
        '4210126996505',
        'f5ba40df24ec63263be31b8cd012de6eb3b0e914',
        1,
        NULL,
        '2023-07-22 19:38:42'
    ), (
        ...
        '+923009223846',
        '42101587694586',
        '87af76aad8672b4ad2382a8c52ed2d99c7e5cd46',
        1,
        NULL,
        '2023-07-26 12:10:23'
    );
...
CREATE TRIGGER `USER_BEFORE_INSERT` BEFORE INSERT ON 
`USER` FOR EACH ROW BEGIN 
	SET NEW.id = GENERATE_ID();
	SET NEW.createdAt = CURRENT_TIMESTAMP();
	IF (NOT VALIDATE_CNIC(NEW.cnic)) THEN SIGNAL SQLSTATE '45000'
	SET
	    MESSAGE_TEXT = 'Invalid CNIC';
	END IF;
	IF ( (NEW.email IS NOT NULL)
	    AND (NOT VALIDATE_EMAIL(NEW.email))
	) THEN SIGNAL SQLSTATE '45000'
	SET
	    MESSAGE_TEXT = 'Invalid email';
	END IF;
	IF (NOT VALIDATE_PHONE(NEW.phone)) THEN SIGNAL SQLSTATE '45000'
	SET
	    MESSAGE_TEXT = 'Invalid phone';
	END IF;
	IF (
	    NOT VALIDATE_PASSWORD(NEW.password)
	) THEN SIGNAL SQLSTATE '45000'
	SET
	    MESSAGE_TEXT = 'Weak password';
	END IF;
	IF (NOT (NEW.role = "WORKER")) THEN SET NEW.areaId = NULL;
	SET NEW.assignedAt = CURRENT_TIMESTAMP();
	ELSE SET NEW.assignedAt = CURRENT_TIMESTAMP();
	END IF;
...
CREATE TRIGGER `USER_BEFORE_UPDATE` BEFORE UPDATE ON 
`USER` FOR EACH ROW BEGIN 
	SET NEW.updatedAt = CURRENT_TIMESTAMP();
	...
	SET
	    MESSAGE_TEXT = 'Invalid email';
	END IF;
	IF ( (NOT NEW.phone = OLD.phone)
	    AND (NOT VALIDATE_PHONE(NEW.phone))
	) THEN SIGNAL SQLSTATE '45000'
	SET
	    MESSAGE_TEXT = 'Invalid phone';
	END IF;
...
CREATE FUNCTION `GENERATE_ID`() RETURNS CHAR(32) CHARSET 
UTF8MB4 BEGIN 
	SET @id := UUID_SHORT();
	RETURN 
@ID; 
...
CREATE FUNCTION `HASH_PASSWORD`(PLAINTEXT TEXT) RETURNS 
VARCHAR(256) CHARSET UTF8MB4 BEGIN 
...
CREATE FUNCTION `VALIDATE_CNIC`(CNIC VARCHAR(45)) RETURNS 
TINYINT BEGIN 
	SET
	    @isValid := REGEXP_LIKE(
	        cnic,
...
CREATE FUNCTION `VALIDATE_EMAIL`(EMAIL VARCHAR(256)
...
CREATE FUNCTION `VALIDATE_PASSWORD`(PASSWORD VARCHAR
(256)) RETURNS INT BEGIN 
	SET
	    @isValid := REGEXP_LIKE(
	        password,
...
CREATE FUNCTION `VALIDATE_PHONE`(PHONE VARCHAR(45)) 
RETURNS INT BEGIN 
	SET
	    @isValid := REGEXP_LIKE(
	        phone,
	        "^\\+92((30|31|32|33|34)[0-9]{1}|355|364)[0-9]{7}$"
	    );
	RETURN 
@ISVALID; 
...
CREATE FUNCTION `VERIFY_ROLE`(P_ID CHAR(32), P_ROLE 
ENUM("GENERAL_SECRETARY", "OFFICE_SECRETARY", "CHAIRPERSON"
, "WORKER", "DONOR")) RETURNS TINYINT BEGIN 
	SET @isVerified := 0;
	SELECT 1
	FROM user
	WHERE role = p_role
	LIMIT 1 INTO @isVerified;
	RETURN 
@ISVERIFIED; 
...
CREATE PROCEDURE `ACCEPT_PENDING_DONATION`(P_DONATIONID 
CHAR(32), P_WORKERID CHAR(32)) BEGIN 
	UPDATE donation
	SET
	    status = "ACCEPTED",
...
CREATE PROCEDURE `ADD_NEW_COLLECTION`(IN P_DONORID 
CHAR(32), IN P_AREAID CHAR(32), IN P_WORKERID CHAR
(32), IN P_AMOUNT FLOAT, IN P_ADDRESS VARCHAR(256)
) BEGIN 
	IF (
	    NOT VERIFY_ROLE(p_donorId, "DONOR")
	) THEN SIGNAL SQLSTATE "45000"
	SET
	    MESSAGE_TEXT = "Donor not found";
	END IF;
	INSERT INTO
	    donation(
	        donorId,
...
CREATE PROCEDURE `ADD_PENDING_DONATION`(IN P_DONORID 
CHAR(32), IN P_AREAID CHAR(32), IN P_AMOUNT FLOAT, 
IN P_ADDRESS VARCHAR(256)) BEGIN 
	IF (
	    NOT VERIFY_ROLE(p_donorId, "DONOR")
	) THEN SIGNAL SQLSTATE "45000"
	SET
...
(IN P_REFNAME VARCHAR(128), IN P_PHONE VARCHAR(45)
, IN P_ADDRESS VARCHAR(256), IN P_AMOUNT FLOAT, IN 
P_AREAID CHAR(32)) BEGIN 
	INSERT INTO
	    donation(
	        refName,
	        refPhone,
...
CREATE PROCEDURE `APPROVE_DONATION_REQUEST`(IN P_DONATIONID 
CHAR(32)) BEGIN 
	UPDATE donation
	SET
	    status = "PENDING",
...
CREATE PROCEDURE `CHANGE_AREA_NAME`(P_ID VARCHAR(32
), P_AREANAME VARCHAR(256)) BEGIN 
	UPDATE area
	SET areaName = p_areaName
	WHERE
	    id = p_id
	    AND active = TRUE
	    AND areaName != p_areaName;
	END 
; 
...
CREATE PROCEDURE `CHANGE_EMAIL`(IN P_ID CHAR(32), IN 
P_EMAIL VARCHAR(256), IN P_EMAILOTP CHAR(4)) BEGIN 
	UPDATE user
	SET
	    email = p_email,
	    emailVerified = FALSE,
...
CREATE PROCEDURE `CHANGE_PASSWORD`(P_ID CHAR(32), P_PASSWORD 
VARCHAR(256), P_NEWPASSWORD VARCHAR(256)) BEGIN 
	UPDATE user
...
CREATE PROCEDURE `CHANGE_PHONE`(IN P_ID CHAR(32), IN 
P_PHONE VARCHAR(45)) BEGIN 
	UPDATE user
	SET phone = p_phone
	WHERE
	    id = p_id
...
CREATE PROCEDURE `COLLECT_ACCEPTED_DONATION`(IN P_DONATIONID 
CHAR(32)) BEGIN 
	UPDATE donation
	SET
	    status = "COLLECTED",
...
CREATE PROCEDURE `CREATE_AREA`(P_AREANAME VARCHAR(256
)) BEGIN 
...
CREATE PROCEDURE `CREATE_USER`(IN P_FIRSTNAME VARCHAR
(45), IN P_LASTNAME VARCHAR(45), IN P_EMAIL VARCHAR
(256), IN P_ROLE ENUM("DONOR", "WORKER", "OFFICE_SECRETARY"
, "CHAIRPERSON", "GENERAL_SECRETARY"), IN P_PHONE 
VARCHAR(45), IN P_CNIC VARCHAR(45), IN P_PASSWORD 
VARCHAR(256), IN P_EMAILOTP VARCHAR(4)) BEGIN 
	INSERT INTO
	    `user` (
	        `firstName`,
	        `lastName`,
...
CREATE PROCEDURE `CREATE_WORKER`(IN P_CHAIRPERSONID 
CHAR(32), IN P_FIRSTNAME VARCHAR(45), IN P_LASTNAME 
VARCHAR(45), IN P_EMAIL VARCHAR(256), IN P_PHONE VARCHAR
(45), IN P_CNIC VARCHAR(45), IN P_PASSWORD VARCHAR
(256), IN P_EMAILOTP VARCHAR(4)) BEGIN 
	SET @areaId = NULL;
	SELECT id
	FROM area
	WHERE
	    chairpersonId = p_chairpersonId
	LIMIT 1 INTO @areaId;
	IF (@areaId IS NULL) THEN SIGNAL SQLSTATE "45000"
	SET
	    MESSAGE_TEXT = "You don't have an area assigned";
	END IF;
	INSERT INTO
	    `user` (
	        `firstName`,
...
CREATE PROCEDURE `DELETE_AREA`(P_ID CHAR(32)) BEGIN 
	CALL UNASSIGN_AREA(p_id);
	UPDATE area SET active = FALSE WHERE id = p_id AND active = TRUE;
	END 
; 
...
CREATE PROCEDURE `DELETE_WORKER`(P_CHAIRPERSONID CHAR
(32), P_WORKERID CHAR(32)) BEGIN 
	SET @areaId := NULL;
	SELECT id
	FROM area
	WHERE
	...
	IF (@areaId IS NULL) THEN SIGNAL SQLSTATE "45000"
	SET
	    MESSAGE_TEXT = "Chairperson not assigned to any area";
	END IF;
	UPDATE user
	SET
	    active = FALSE,
	...
	SELECT
	    CASE
	        ...
	        ELSE 0
	    END AS updated;
	IF (NOT updated) THEN SIGNAL SQLSTATE "45000"
	SET
	    MESSAGE_TEXT = "Worker not found in chairperson's area";
	END IF;
	END 
; 
...
CREATE PROCEDURE `DONOR_DONATION_REQUEST`(IN P_DONORID 
CHAR(32), IN P_AREAID CHAR(32), IN P_AMOUNT FLOAT, 
IN P_ADDRESS VARCHAR(256)) BEGIN 
	IF (
	    NOT VERIFY_ROLE(p_donorId, "DONOR")
...
CREATE PROCEDURE `REMOVE_EMAIL`(IN P_ID CHAR(32)) BEGIN 
	UPDATE user
	SET
	    email = NULL,
	    emailVerified = FALSE,
...
CREATE PROCEDURE `RESET_PASSWORD`(CREDENTIAL CHAR(32
), P_PASSWORD VARCHAR(256)) BEGIN 
	UPDATE user
...
CREATE PROCEDURE `UNASSIGN_AREA_ =REQUIRE(_CHAIRPERSON`
(P_AREAID CHAR(32)) BEGIN 
	UPDATE area
	SET
	    chairpersonId = NULL,
...
CREATE PROCEDURE `VERIFY_CREDENTIALS`(IN CREDENTIAL 
VARCHAR(256), IN P_PASSWORD VARCHAR(45), IN PLATFORM 
ENUM("WEB", "MOBILE")) BEGIN 
...
CREATE PROCEDURE `VERIFY_EMAIL`(P_ID CHAR(32), P_EMAILOTP 
VARCHAR(4)) BEGIN 
	UPDATE user
	SET
	    emailVerified = 1,
...
`ACCEPTED_DONATIONS` AS 
	select
	    ...
	    `d`.`refName` AS `refName`,
	    `d`.`refPhone` AS `refPhone`,
	    `d`.`areaId` AS `areaId`,
	    `d`.`createdAt` AS `createdAt`,
	    `d`.`updatedAt` AS `updatedAt`,
	    `d`.`requestedAt` AS `requestedAt`,
	    `d`.`approvedAt` AS `approvedAt`,
	    `d`.`acceptedAt` AS `acceptedAt`,
	...
	FROM (
	        `donation` `d`
	        left join `user_data` `u` on( (`u`.`id` = `d`.`donorId`))
	    )
...
`ACTIVE_AREAS` AS 
	select
	    ...
	    `area`.`chairpersonId` AS `chairpersonId`,
	    `area`.`active` AS `active`,
	    `area`.`assignedAt` AS `assignedAt`
	FROM `area`
...
CREATE ALGORITHM =UNDEFINED SQL SECURITY DEFINER VIEW 
`AREA_WITH_CHAIRPERSON` AS 
	select
	    `a`.`id` AS `id`,
	    `a`.`createdAt` AS `createdAt`,
	    `a`.`updatedAt` AS `updatedAt`,
	    `a`.`areaName` AS `areaName`,
	    `a`.`chairpersonId` AS `chairpersonId`,
	    `a`.`active` AS `active`,
	    ...
	    `ch`.`email` AS `chairpersonEmail`,
	    `ch`.`phone` AS `chairpersonPhone`
	FROM (
	        `area` `a`
	        left join `user_data` `ch` on( (
	                `a`.`chairpersonId` = `ch`.`id`
	            )
	        )
	    )
; 
...
`CHAIRPERSON_DATA` AS 
	select
	    ...
	    `a`.`id` AS `areaId`,
	    `a`.`createdAt` AS `areaCreatedAt`,
	    `a`.`updatedAt` AS `areaUpdatedAt`,
	    `a`.`areaName` AS `areaName`,
	    `a`.`assignedAt` AS `areaAssignedAt`
	FROM (
	        `user_data` `c`
	        left join `area` `a` on( (`c`.`id` = `a`.`chairpersonId`)
	        )
	    )
	where (`c`.`role` = 'CHAIRPERSON')
; 
...
`DONATION_INFO` AS 
	select
	    ...
	    `c`.`phone` AS `chairpersonPhone`,
	    `c`.`cnic` AS `chairpersonCnic`,
	    `c`.`email` AS `chairpersonEmail`,
	    concat(
	        `dn`.`firstName`,
	        ' ',
	        `dn`.`lastName`
	    ...
	    `w`.`cnic` AS `workerCnic`,
	    `w`.`email` AS `workerEmail`
	FROM ( ( ( ( (
	                        `donation` `d`
	                        left join `area` `a` on( (`a`.`id` = `d`.`areaId`))
	                    )
	                    left join `user_data` `c` on( (`c`.`id` = `a`.`chairpersonId`)
	                    )
	                )
	                left join `user_data` `u` on( (`u`.`id` = `d`.`donorId`))
	            )
	            left join `user_data` `w` on( (`w`.`id` = `d`.`workerId`))
	        )
	        left join `user_data` `dn` on( (`dn`.`id` = `d`.`donorId`))
	    )
; 
...
`DONOR_HISTORY` AS 
	...
	FROM ( (
	            `donation` `d`
	            left join `user_data` `u` on( (`u`.`id` = `d`.`id`))
	        )
	        left join `area` `a` on( (`a`.`id` = `d`.`areaId`))
	    )
	where (`d`.`status` = 'COLLECTED')
...
`REQUESTED_DONATIONS` AS 
	...
	FROM (
	        `donation` `d`
	        join `user_data` `u` on( (`u`.`id` = `d`.`donorId`))
	    )
...
`USER_DATA` AS 
	select
	    ...
	    `user`.`areaId` AS `areaId`,
	    `user`.`assignedAt` AS `assignedAt`
	FROM `user`
; 
...
-- Dump completed on 2023-07-27  0:55:09